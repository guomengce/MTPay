...
import type { ManualBalanceAdjustmentDetail, TransactionBusinessType, TransactionInfoResult } from '@/api/modules/transaction';
...
const manualDetail=computed(()=>info.value && ['manual_increase','manual_decrease'].includes(info.value.transaction.business_type)?info.value.detail as ManualBalanceAdjustmentDetail:null);
function goBack(){void router.push('/records')}
onMounted(async()=>{const businessType=route.params.businessType as TransactionBusinessType;const businessId=Number(route.params.businessId);if(!['deposit','exchange','withdrawal','manual_increase','manual_decrease'].includes(businessType)||!Number.isInteger(businessId)||businessId<=0){invalid.value=true;return}loading.value=true;try{info.value=await TransactionApi.fetchTransactionInfo({business_type:businessType,business_id:businessId})}finally{loading.value=false}});
</script>
<style scoped lang="scss">.transaction-detail{display:grid;min-width:0;gap:18px;padding:20px}.transaction-detail>.el-button{width:fit-content}.transaction-detail__content{min-width:0;min-height:300px}@include mobile{.transaction-detail{padding:0;gap:14px}}</style>
