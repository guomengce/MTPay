<template>
  <div class="withdrawal-detail-content">
    ...
    <Timeline v-if="detail.records.length" :records="detail.records" :loading="fileLoading" @preview="emit('preview', $event)" @download="emit('download', $event)" />
  </div>
</template>
...
import OrderHeader from './OrderHeader.vue'; import ReviewCard from './ReviewCard.vue'; import TransactionOverview from './TransactionOverview.vue'; import Timeline from './Timeline.vue';
...
@include mobile {
  .withdrawal-detail-content { gap: 14px; }
}
</style>
