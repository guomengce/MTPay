<template>
  <DetailCard class="withdrawal-timeline" :title="t('withdrawal.timeline')" icon="ri-time-line">
        ...
        <article>
          ...
          <div v-if="record.files.length" class="timeline-files">
            <p class="timeline-files__count">
              {{ t('withdrawal.associatedFiles', { count: record.files.length }) }}
            </p>
            <div class="file-list">
              <article v-for="file in record.files" :key="file.file_id">
                ...
                <div class="file-list__info">
                  ...
                  <p>
                    {{ file.file_type_name || t('withdrawal.proofFile') }} ·
...
.file-list__actions :deep(.el-button.is-circle) {
  width: 32px;
  min-width: 32px;
  height: 32px;
  padding: 0;
